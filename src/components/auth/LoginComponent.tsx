...
const LoginComponent: React.FC<LoginComponentProps> = ({ initialScreen = 'login', onLoginSuccess }) => {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      const success = await login(mobile, pinStr); // Assuming PIN is used as password
      if (success) {
  ...
  const handleForgot = async () => {
    ...
    try {
      ...
      const response = await import('@/utils/auth').then(m => m.forgotPassword(forgotMobile));
