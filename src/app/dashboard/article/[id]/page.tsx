...
import { Layout, Typography, Tag, Skeleton, Button, message, Space } from 'antd';
  ...
  useEffect(() => {
    const fetchArticle = async () => {
      try {
        ...
        // Enhance article content with detailed information
        ...
        <Layout style={layoutStyle}>
          <Content style={{ padding: '24px', marginTop: 64 }}>
            <div style={{ maxWidth: '800px', margin: '0 auto', textAlign: 'center' }}>
              <Title level={2} style={{ color: darkMode ? '#fff' : '#000000' }}>Article not found</Title>
    ...
    <ProtectedRoute>
      <Layout style={layoutStyle}>
            ...
            <div style={{ 
              ...
              >
                {article.content.split('\n').map((paragraph, index) => {
                  ...
                  // Check if it's bold text (standalone)
        ...
        <Footer style={{ textAlign: 'center', color: darkMode ? 'rgba(255, 255, 255, 0.4)' : 'rgba(0, 0, 0, 0.4)', background: 'transparent', marginTop: '48px' }}>
          Secure Portal ©{new Date().getFullYear()} - Article Detail
        </Footer>
      </Layout>
    </ProtectedRoute>
  );
